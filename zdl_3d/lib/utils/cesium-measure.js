    ...
    Cesium.Measure = (function (Cesium) {
        ...
        _.prototype = {
            ...
            getCatesian3FromPX: function (px) {
                if (this._viewer && px) {
                    return this._viewer.scene.pickPosition(px);
                }
            },
            // getCatesian3FromPX: function (px) {
            //   if (this._viewer && px) {
            ...
            drawAreaMeasureGraphics: function (options = {}) {
                if (this._viewer && options) {
                    var positions = [],
                        ...
                        polyObj = null,
                        _label = '',
                        _handler = createScreenSpaceEventHandler(this._viewer.scene.canvas);
                    // left
            ...
            drawTrianglesMeasureGraphics: function (options = {}) {
                ...
                if (this._viewer && options) {
                    ...
                    _handler.setInputAction(function (movement) {
                        ...
                        $this._drawLayer.entities.remove(_tempLineEntity2);
                        $this._drawLayer.entities.remove(_tempLineEntity);
                        $this._drawLayer.entities.remove(_trianglesEntity);
                    }, Cesium.ScreenSpaceEventType.RIGHT_CLICK);
                    // create entity
                    //直线
                    ...
                    _tempLineEntity2 = this._drawLayer.entities.add(_tempLineEntity2)
                    _tempLineEntity = this._drawLayer.entities.add(_tempLineEntity)
                    _trianglesEntity = this._drawLayer.entities.add(_trianglesEntity)
                }
            },
            /**
             * 测水平距离
             * @param {*} options
             */
            drawHorizontalMeasureGraphics: function (options = {}) {
                options.style = options.style || {
                    width: 3,
                    material: Cesium.Color.BLUE.withAlpha(0.5)
                }
                if (this._viewer && options) {
                    ...
                    _tempLineEntity2.label = {
                        text: new Cesium.CallbackProperty(function () {
                            if (isHorizontal) {
                                return '水平距离:' + $this.getPositionDistance($this.transformCartesianArrayToWGS84Array(_tempPoints)) + '米'
                            } else {
                                return '水平距离:' + $this.getPositionDistance($this.transformCartesianArrayToWGS84Array(_tempPoints2)) + '米'
                            }
                        }, false),
             ...
             * @param {*} options
             */
            drawVerticalMeasureGraphics: function (options = {}) {
                options.style = options.style || {
                    width: 3,
                    material: Cesium.Color.BLUE.withAlpha(0.5)
                }
                if (this._viewer && options) {
                    ...
                    function _getHeading(startPosition, endPosition) {
                        ...
                        let cartographic = Cesium.Cartographic.fromCartesian(startPosition);
                        let cartographic2 = Cesium.Cartographic.fromCartesian(endPosition);
                        return Math.abs(cartographic2.height - cartographic.height).toFixed(2)
                    }
                    ...
                    _tempLineEntity2.label = {
                        text: new Cesium.CallbackProperty(function () {
                            if (isHorizontal) {
                                return '垂直距离:' + _getHeading(_tempPoints2[0], _tempPoints2[1]) + '米'
                            } else {
                                return '垂直距离:' + _getHeading(_tempPoints[0], _tempPoints[1]) + '米'
                            }
                        }, false),
                        show: true,
                        showBackground: true,
