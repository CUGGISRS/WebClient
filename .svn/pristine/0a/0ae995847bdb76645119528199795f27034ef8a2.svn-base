...
define(['exports', './when-cbf8cd21', './Check-35e1a91d', './Math-9c31f76e', './Cartesian2-cb461156', './Transforms-4d469d4c', './ComponentDatatype-607c9a0c', './GeometryAttribute-b43261ff', './AttributeCompression-7d25f605', './EncodedCartesian3-1b7d0747', './IndexDatatype-0dca33ea', './IntersectionTests-e96b6232', './Plane-c86889a2'], function (exports, when, Check, _Math, Cartesian2, Transforms, ComponentDatatype, GeometryAttribute, AttributeCompression, EncodedCartesian3, IndexDatatype, IntersectionTests, Plane) { 'use strict';
  ...
  GeometryPipeline.compressVertices = function(geometry) {
    ...
    var strAttribute = geometry.attributes.str;

    var hasNormal = when.defined(normalAttribute);
    var hasSt = when.defined(stAttribute);
    var hasStr = when.defined(strAttribute);
    if (!hasNormal && !hasSt && !hasStr) {
      return geometry;
    }
    ...
    var length = hasNormal ? normals.length : st ? st.length : str.length;
    ...
    var numCompressedComponents = hasNormal ? 1.0 : 0.0;
    if (hasStr) {
      numCompressedComponents += 2;
    } else if (hasSt || !hasNormal) {
      numCompressedComponents += 1;
    }
    ...
    for (i = 0; i < numVertices; ++i) {
      ...
      if (hasStr) {
        Cartesian2.Cartesian2.fromArray(str, i * 3.0, scratchCartesian2$1);
        compressedAttributes[
          normalIndex++
          ] = AttributeCompression.AttributeCompression.compressTextureCoordinates(scratchCartesian2$1);
        Cartesian2.Cartesian2.fromArray(str, i * 3.0 + 1, scratchCartesian2$1);
        scratchCartesian2$1.x = 0;
        compressedAttributes[
          normalIndex++
          ] = AttributeCompression.AttributeCompression.compressTextureCoordinates(scratchCartesian2$1);
    ...
    if (hasStr) {
      delete geometry.attributes.str;
    }
    if (hasBitangent) {
...
//# sourceMappingURL=GeometryPipeline-76b92a4a.js.map
