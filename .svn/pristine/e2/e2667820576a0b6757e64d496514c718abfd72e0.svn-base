...
public abstract class BaseBiz<M extends CommonMapper<T>, T> {
     ...
     * 通过某一属性的属性名和属性值获取数据
     */
    public List<T> getListByFiled(Object fieldValue,String fieldName){
     ...
     * @return
     */
    public int deleteDataByFiled(Object filed,String filedName){
        Map<String,Object> toMap=new HashMap<>();
        toMap.put(filedName,filed);
     ...
     * 通过某一属性的属性名和属性值修改数据
     */
    public int updateByFiled(T t,Object filed,String filedName){
     ...
     * @param total
     */
    public void mapPutPageLimitTotal(Map<String, Object> params, int total){
       int page=1;
       int limit=10;
        Object page0=params.get("page");
        Object limit0=params.get("limit");
        if(page0!=null&&!"".equals(page0.toString().trim())) {
            page = Integer.parseInt(page0.toString());
        }
        if(limit0!=null&&!"".equals(limit0.toString().trim())) {
            limit = Integer.parseInt(limit0.toString());
        }
        int countPage= (int) Math.ceil((double)total/limit);
        if(page>countPage){
            page=countPage;
        }
        if(page<1){
     ...
     * @return
     */
    public List<List<T>> importExcel(InputStream in,String fileName,T entity) throws Exception {
        return ExcelUtils.importExcel(in,fileName,entity);
    }
     ...
     * @throws Exception
     */
    public void exportExcel(HttpServletRequest request, HttpServletResponse response, String[] excelHeader, String sheetName, List<T> list, String fileName) throws Exception {
        ExcelUtils.exportExcel(request,response,excelHeader,sheetName,list,fileName);
    }
     ...
     * @return
     */
    public TableResultResponse<T> getListByPageAndCondition(Query query, Example example){
        ...
        //一种分页方式，mybatis 自带的 RowBounds
        RowBounds rowBounds=new RowBounds((page-1)*limit,limit);
        return new TableResultResponse<T>(total, mapper.selectByExampleAndRowBounds(example,rowBounds));
    }
