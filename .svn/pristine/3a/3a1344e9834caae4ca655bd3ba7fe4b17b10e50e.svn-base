...
define(['exports', './when-cbf8cd21', './Check-35e1a91d', './Math-9c31f76e', './Cartesian2-cb461156'], function (exports, when, Check, _Math, Cartesian2) { 'use strict';
   ...
   * Pack texture coordinates into a single float. The texture coordinates will only preserve 12 bits of precision.
   *
   * @param {Cartesian3} texture3DCoordinates The texture coordinates to compress.  Both coordinates must be in the range 0.0-1.0.
   * @returns {Number} The packed texture coordinates.
   *
   */
  AttributeCompression.compressTexture3DCoordinates = function(
    texture3DCoordinates
  ) {
    //>>includeStart('debug', pragmas.debug);
    Check.Check.defined("texture3DCoordinates", texture3DCoordinates);
    //>>includeEnd('debug');

    // Move x and y to the range 0-4095;
    var x = (texture3DCoordinates.x * 3072.0) | 0;
    var y = (texture3DCoordinates.y * 3072.0) | 0;
    var z = (texture3DCoordinates.z * 3072.0) | 0;
    return 4096.0 * 4096.0 * x + 4096.0 * y + z;
  };
   ...
   *
   * @param {Number} compressed The compressed texture coordinates.
   * @param {Cartesian3} result The decompressed texture coordinates.
   * @returns {Cartesian3} The modified result parameter.
   *
   */
  AttributeCompression.decompressTexture3DCoordinates = function(
    compressed,
    result
  ) {
    //>>includeStart('debug', pragmas.debug);
    ...
    var temp = compressed / 4096.0;
    var xyZeroTo4095 = Math.floor(temp);
    result.z = (compressed - xyZeroTo4095 * 4096.0) / 3072.0;
    temp = xyZeroTo4095 / 4096.0;
    var xZeroTo4095 = Math.floor(temp);
    result.x = xZeroTo4095 / 3072.0;
    result.y = (xyZeroTo4095 - xZeroTo4095 * 4096.0) / 3072.0;
    if (result.x > 1.0) {
      result.x = 1.0;
    }
    if (result.y > 1.0) {
...
//# sourceMappingURL=AttributeCompression-7d25f605.js.map
