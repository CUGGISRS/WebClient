...
public class OkHttpTokenInterceptor implements Interceptor {
    ...
    public Response intercept(Chain chain) throws IOException {
        ...
        if (chain.request().url().toString().contains("client/token")) {
            ...
            String uH=userAuthConfig.getTokenHeader();
            String bT= BaseContextHandler.getToken();
            String sH=serviceAuthConfig.getTokenHeader();
            String sT= serviceAuthUtil.getClientToken();
            newRequest = chain.request()
                    .newBuilder()
                    .header(uH,bT)
                    .header(sH, sT)
