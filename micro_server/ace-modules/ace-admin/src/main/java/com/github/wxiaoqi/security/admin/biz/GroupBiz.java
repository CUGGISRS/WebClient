...
public class GroupBiz extends BaseBiz<GroupMapper, Group> {
    ...
    public void modifyGroupUsers(int groupId, String members, String leaders) {
       groupLeaderMapper.deleteByGroupId(groupId);
       groupMemberMapper.deleteByGroupId(groupId);
        if (!StringUtils.isEmpty(members)) {
            String[] mem = members.split(",");
            for (String m : mem) {
               groupMemberMapper.insertByGroupIdAndUserId(groupId, Integer.parseInt(m));
            }
        }
        if (!StringUtils.isEmpty(leaders)) {
            String[] mem = leaders.split(",");
            for (String m : mem) {
                groupLeaderMapper.insertByGroupIdAndUserId(groupId, Integer.parseInt(m));
            }
        }
    }
